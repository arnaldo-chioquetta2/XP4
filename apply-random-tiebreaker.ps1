...
function Backup-File($path) {
    if (-not (Test-Path $path)) { Write-Error ("Arquivo não encontrado: {0}" -f $path); return $false }
    $bak = "{0}.bak.{1}" -f $path, (Get-Date).ToString('yyyyMMddHHmmss')
...
if ($trackText -match "RandomTieBreaker") {
    ...
    if ($WhatIf) {
        ...
        $newTrackText = $trackText -replace [regex]::Escape($pattern), ($insert)
